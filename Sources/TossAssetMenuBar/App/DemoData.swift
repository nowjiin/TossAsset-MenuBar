...
enum DemoData {
    ...
    /// 종목별 직전 거래일 종가. 오늘 등락률 표시를 데모에서도 보이게 한다.
    /// 실제로는 `/candles` 로 받아오는 값이다.
    static let basePrices: [String: TossDecimal] = [
        "005930": "71000",   // 현재가 72,000 → +1.41%
        "000660": "196000",  // 현재가 192,500 → -1.79%
        "AAPL": "176.20",    // 현재가 178.50 → +1.31%
        "NVDA": "121.90",    // 현재가 120.40 → -1.23%
    ]
